...
/** 需要 overlay 的四个类别 */
const OVERRIDE_CATEGORIES = ['instruct', 'context', 'sysprompt', 'reasoning'];
// genParams 键名列表
...
const CONTEXT_GLOBAL_KEYS = ['always_force_name2', 'trim_sentences', 'single_line'];
...
const NSFW_ORIGINAL_REF = Symbol.for('nsfw-auto-model-switcher.original-ref');
 ...
 */
function createOverlay(originalObj, overrides) {
    const overlay = { ...originalObj, ...overrides };
 ...
 */
export function initPresetOverrides() {
    purgeLegacyMarker();
    ...
    for (const cat of OVERRIDE_CATEGORIES) {
        ...
        if (current && typeof current === 'object' && current[NSFW_ORIGINAL_REF]) {
            pu[cat] = current[NSFW_ORIGINAL_REF];
            restored++;
        }
    }
    if (restored > 0) {
 ...
 * 从预设中提取生成参数
 */
export function extractGenParams(preset) {
    if (!preset)
        return null;
    const params = {};
...
//# sourceMappingURL=preset-proxy.js.map