...
import { escapeHtml, setRenderCallback, renderLogEntryHtml, renderLogsHtml, getLogs, clearLogs } from './logger.js';
import { loadSettings, getSettingsRoot, collectAndSaveFromDom, updateStatusIndicator, getAllPresetNames, getActivePreset, getActivePresetName, savePresetAs, deletePreset, renamePreset, exportPreset, } from './settings.js';
...
export function setupLogRendering() {
    setRenderCallback(function (logs, newLog) {
        ...
        if (newLog && shouldShowLog(newLog, minLevel)) {
            ...
            while ($c.children().length > maxEntries)
...
// ===== 设置面板事件绑定 =====
export function bindSettingsListeners($panel, deps) {
    // 折叠/展开预设区域
    ...
    $panel.on('input change', '#nsfw_switcher_enabled, #nsfw_switcher_api_url, #nsfw_switcher_api_key, ' +
        ...
        '#nsfw_switcher_api_retries, #nsfw_switcher_safety_timeout', function () {
    ...
    $panel.on('click', '#nsfw_switcher_restore_btn', async function () {
        ...
        if (typeof toastr !== 'undefined')
            toastr.success('[NSFW 模型切换器] 已恢复原始模型');
        updateIndicator();
    });
...
//# sourceMappingURL=ui-bindings.js.map