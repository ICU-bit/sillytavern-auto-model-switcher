...
function createDialogElement(type, message, inputDefault) {
    ...
    if (type === 'confirm') {
        dialog._overlay._escapeHandler = function (e) {
            if (e.key === 'Escape') {
...
function destroyDialog(overlay) {
    ...
    const dialog = overlay.querySelector('.nsfw-modal-dialog');
    if (dialog)
...
function collapseModuleFields(container, moduleId) {
    ...
    if (header) {
        ...
        if (chevron)
            chevron.classList.remove('nsfw-expanded');
    }
}
function expandModuleFields(container, moduleId) {
    ...
    if (header) {
        const chevron = header.querySelector('.nsfw-collapse-icon');
        if (chevron)
            chevron.classList.add('nsfw-expanded');
    }
}
