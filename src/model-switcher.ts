...
const SOURCE_TO_API_URL_FIELD: Record<string, string> = {
    'custom': 'custom_url',
};
...
interface SettingsSnapshot {
    ...
    presets: {
        instruct: object | null;
        context: object | null;
        sysprompt: object | null;
...
let lastSwitchTargetSource: string | null = null;
...
function takeSnapshot(): SettingsSnapshot | null {
    ...
    try {
        ...
        const apiKeyField = source + '_api_key';
...
export function getCurrentModelInfo(): CurrentModelInfo | null {
    ...
    if (field && oai_settings[field]) {
        return {
            model: String(oai_settings[field]),
            source: source,
        };
    }
    return null;
...
export function saveSettingsSnapshot(): boolean {
    if (settingsSnapshot) {
        return true;
    }
    ...
    if (settingsSnapshot) {
        ...
        if (presets?.instruct) {
            const i = presets.instruct as Record<string, unknown>;
            const c = presets.context as Record<string, unknown> | null;
            const s = presets.sysprompt as Record<string, unknown> | null;
            addLog('已保存预设: instruct=' + (i.preset || '无') + ', context=' + (c?.preset || '无') + ', sysprompt=' + (s?.name || '无'), 'info');
        }
        return true;
    }
...
export async function switchToModel(
    ...
    try {
        ...
        const msg = e instanceof Error ? e.message : String(e);
        addLog('切换模型失败: ' + msg, 'error');
        return false;
    }
}

export async function restoreOriginalModel(): Promise<boolean> {
    if (!settingsSnapshot) {
