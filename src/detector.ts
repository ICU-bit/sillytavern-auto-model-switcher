...
/** OpenAI 兼容 chat completion 响应（仅本模块关心的字段） */
 ...
 */
export function getLastAiMessageText(): string | null {
    try {
...
export function getMessageTextById(messageId: number): string | null {
    try {
        const context = getContext();
        const chat = context?.chat || [];
 ...
 */
export async function testNsfwApi(): Promise<boolean | null> {
