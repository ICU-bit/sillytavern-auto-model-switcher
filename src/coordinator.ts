...
/** 已应用的覆盖数据三件套 (enable 时需要的全部参数) */
export interface AppliedOverrides {
    /** 预设原始数据 (用于 Proxy 包装) */
    presetData: PresetData;
    /** 字段开关字典 (哪些字段参与覆盖) */
    mods: PresetModuleEnabledMap;
    /** 生成参数 (temperature/top_p 等, 直接写入 direct-api 请求体) */
    genParams: Record<string, unknown> | null;
}

/**
 * 协调器运行时状态 (discriminated union)
 *
 * - idle:     三层全关
 * - switched: fetch ON + Proxy ON + presetOverrides 设置
 * - partial:  fetch ON + Proxy OFF (Path A 后, ST 格式化已完, 等待重定向请求)
 */
export type CoordRuntime =
    | { kind: 'idle' }
    | { kind: 'switched'; applied: AppliedOverrides }
    | { kind: 'partial'; applied: AppliedOverrides };

/** disable 的语义原因 (用于日志 + state 通知策略) */
export type DisableReason =
    | 'restore'         // 正常恢复 (onGenerationStarted restore 路径)
    | 'manual'          // 用户手动按钮
    | 'plugin_off'      // 用户关闭插件总开关
    | 'safety_timeout'  // 30s safety timer 触发
    | 'fetch_fallback'; // fetch 兜底 (飞行请求时插件被禁用)

// ===== Coordinator =====

export class SwitcherCoordinator {
    private state: ModelStateMachine;

    /** 当前运行时状态 */
    private runtime: CoordRuntime = { kind: 'idle' };
     ...
     */
    private lastPrepared: AppliedOverrides | null = null;

    /** state.onTransition 取消订阅函数 (供 dispose 用) */
    private unsubscribe: (() => void) | null = null;

    constructor(state: ModelStateMachine) {
        this.state = state;
    }
     ...
     * 准备好的预设。
     */
    prepare(overrides: AppliedOverrides): void {
    ...
    disable(reason: DisableReason): void {
        if (this.runtime.kind === 'idle') return;
     ...
     */
    onPayloadCaptured(): void {
        if (this.runtime.kind !== 'switched') return;
     ...
     * 处理状态转换事件 (核心自动驱动逻辑)
     */
    private handleTransition(ctx: TransitionContext): void {
 ...
 */
export function createCoordinator(state: ModelStateMachine): SwitcherCoordinator {
