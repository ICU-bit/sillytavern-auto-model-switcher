...
export function initFetchInterceptor(): void {
    ...
    window.fetch = async function (input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
        ...
        if (interceptEnabled && isStApiEndpoint(url)) {
            ...
            if (aborted) {
                // 剥离 init.signal
                const cleanInit: RequestInit = init ? { ...init, signal: undefined } : {};
                // 剥离 input.signal (Request 实例时, 用 Request 拷贝构造)
                // 注: 直接传 Request 给 fetch 时 signal 由 Request 自身携带, 必须替换 Request
                const cleanInput: RequestInfo | URL = input instanceof Request
 ...
 */
export function restoreFetchInterceptor(): void {
    if (originalFetch) {
...
async function redirectToTarget(
    ...
    const onStAbort = stSignal
        ? function () { if (currentController) currentController.abort(); }
        : null;
